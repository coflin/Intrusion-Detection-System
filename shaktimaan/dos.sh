...
do
	...
	then
		echo ${ip_array[$i]}
		sudo timeout 10 sudo tshark -i wlan0 -Y "ip.src == `hostname -I | awk '{print $1}'` && ip.dst == ${ip_array[$i]}" > /etc/Intrusion-Detection-System/shaktimaan/logs/DOS	
		grep -a SYN /etc/Intrusion-Detection-System/shaktimaan/logs/DOS
		if [ $? -eq 0 ]
		then
